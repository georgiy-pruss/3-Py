...
import re,random,time,os,urllib.request,urllib.error
...
def geturl(u): # see http://docs.python.org/py3k/howto/urllib2.html
  try: return urllib.request.urlopen(u).read()
...
int.i = float.f = bytes.b = property(lambda x:x) # self, to allow s.b and b.b, etc
int.s = float.s = int.s = property(str)
int.abs = float.abs = property(abs)
